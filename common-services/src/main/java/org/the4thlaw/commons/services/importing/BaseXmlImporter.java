...
public abstract class BaseXmlImporter<D extends IDatabaseDao> implements IImporter {
	...
	@Override
	public boolean supports(String originalFilename, Path file) throws CommonException {
	 ...
	 * @throws CommonException in case of error during extraction.
	 */
	protected Path extractZip(Path file) throws CommonException {
		try {
	...
	@Override
	public void importFile(String originalFilename, Path file) throws CommonException {
		...
		try {
			...
			throw new CommonException(CommonErrorCode.IMPORT_IO_ERROR, ioe);
		} catch (Exception e) {
			if (e.getCause() instanceof CommonException de) {
				// Rethrow exceptions of the right type
				throw de;
			}
			throw new CommonException(CommonErrorCode.IMPORT_PARSE_ERROR, e);
		} finally {
	 ...
	 * @throws CommonException If moving the images fails.
	 */
	protected void restoreImages(Path archiveDirectory, String imagesDirectoryName) throws CommonException {
		File systemDirectory = directoryService.getImagesDirectory().toFile();
		...
		try {
			...
			throw new CommonException(CommonErrorCode.IMPORT_IMAGES_ERROR, e);
		}
