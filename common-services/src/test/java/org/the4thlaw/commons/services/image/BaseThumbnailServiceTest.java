...
/**
 * Tests from {@link BaseThumbnailService}.
 */
class BaseThumbnailServiceTest {
    ...
    static class ThumbnailService extends BaseThumbnailService {
        protected ThumbnailService(Path thumbnailDirectory, int queueSize) {
            super(thumbnailDirectory, queueSize);
        }
	...
	private Path copyImage(String name) throws IOException, URISyntaxException {
		URL url = BaseThumbnailServiceTest.class.getResource("/" + name);
	...
	void getThumbnailGenerated(String imageName) throws IOException, URISyntaxException {
	...
	@ValueSource(strings =
	{ "image-600x800.jpg", "image-600x800.png" })
	void getThumbnailOriginal(String imageName)			throws IOException, URISyntaxException {
		Path imagePath = copyImage(imageName);
	...
	@ValueSource(strings =
	{ "image-600x800.jpg", "image-600x800.png" })
	void getThumbnailLenient(String imageName)			throws  IOException, URISyntaxException {
		Path imagePath = copyImage(imageName);
	...
	@ValueSource(strings =
	{ "image-600x800.jpg", "image-600x800.png" })
	void getThumbnailFallback(String imageName)			throws  IOException, URISyntaxException {
		Path imagePath = copyImage(imageName);
	...
	private static File assertThumbnailGenerated(BaseThumbnailService service, Path imagePath, int width)
			throws  IOException {
